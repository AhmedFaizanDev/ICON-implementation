...
# Fallback used when the target model returns empty/null for setup turns 1 & 2.
# Empty assistant messages in conversation history cause API rejections on some providers.
_SETUP_FALLBACK = "Understood, please continue."
...
class MultiTurnAttacker:
    ...
    def attack(
        ...
        response_1 = self.llm_client.generate(prompt_1, max_tokens=400)
        if not response_1 or not response_1.strip():
            response_1 = _SETUP_FALLBACK
        ...
        if not response_2 or not response_2.strip():
            response_2 = _SETUP_FALLBACK
