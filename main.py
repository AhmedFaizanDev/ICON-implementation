...
class UnifiedJailbreakFramework:
    ...
    def _try_attack_method(
        ...
        try:
            ...
            # Always request dict result so .get() calls below work regardless of sample_index
            attack_doc_result = self._generate_with_template(
                generator, pattern, harmful_query, is_attack=True, return_details=True
            )
            ...
            # Get document based on pattern (attack_doc_result is always a dict)
            if pattern == "Scientific Research":
                ...
                non_meta_keys = [k for k in attack_doc_result if k not in ('prompt', 'response', 'from_template')]
                attack_document = attack_doc_result.get(non_meta_keys[0], '') if non_meta_keys else ''
    ...
    def execute(self, harmful_query: str, output_file: Optional[str] = None, sample_index: Optional[int] = None, cost_tracker: Optional[CostTracker] = None, prompt_recorder: Optional[PromptRecorder] = None) -> dict:
        ...
        """
        # Reset attacker history at the start of every new sample
        self.attacker.reset()
        ...
        # Truncate forged document to keep total request under Groq's 6000 TPM limit.
        # ~4 chars per token → 12000 chars ≈ 3000 tokens, leaves room for prompts+history.
        _MAX_DOC_CHARS = 4000
        if isinstance(forged_document, str) and len(forged_document) > _MAX_DOC_CHARS:
            forged_document = forged_document[:_MAX_DOC_CHARS] + "\n[... document truncated for length ...]"
            results['forged_document'] = forged_document
        ...
        if results['jailbreak_response'] and not results.get('judge_result'):
            ...
            try:
                ...
                # Use template manager to generate attack method document (always dict)
                attack_doc_result = self._generate_with_template(
                    generator, pattern, harmful_query, is_attack=True, return_details=True
                )
                ...
                # Truncate attack document too
                if isinstance(attack_document, str) and len(attack_document) > _MAX_DOC_CHARS:
                    attack_document = attack_document[:_MAX_DOC_CHARS] + "\n[... document truncated for length ...]"
